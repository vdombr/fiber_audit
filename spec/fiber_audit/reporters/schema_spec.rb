...
RSpec.describe FiberAudit::Reporters::Schema do
  ...
  describe '.validate!' do
    context 'with valid result' do
      it 'returns a frozen hash with correct structure' do
        hash = described_class.validate!(result)
    ...
    context 'with missing interface methods' do
      it 'raises ReporterError when result missing findings' do
        ...
        expect { described_class.validate!(bad_result) }.to raise_error(
    ...
    context 'with invalid status' do
      ...
      it 'accepts all allowed statuses' do
        %w[FAIL REVIEW PASS_WITH_WARNINGS NO_FINDINGS].each do |status|
          ...
          expect { described_class.validate!(good_result) }.not_to raise_error
        end
      end
    end
    ...
    context 'with empty evidence' do
      it 'raises ReporterError for finding with no evidence' do
        ...
        bad_result = double('result', findings: [finding_no_evidence], suppressed: [], parse_errors: [], coverage: coverage, status: 'FAIL')
        expect { described_class.validate!(bad_result) }.to raise_error(
          FiberAudit::ReporterError,
    ...
    context 'with invalid severity' do
      it 'raises ReporterError for unknown severity' do
        finding_bad_sev = FiberAudit::Finding.new(
          rule_id: 'test',
          category: 'test',
          severity: :critical,
          confidence: :confirmed,
        ...
        # Manually override severity to invalid value
        finding_bad_sev.instance_variable_set(:@severity, :unknown)
        bad_result = double('result', findings: [finding_bad_sev], suppressed: [], parse_errors: [], coverage: coverage, status: 'FAIL')
        expect { described_class.validate!(bad_result) }.to raise_error(
          FiberAudit::ReporterError,
          /invalid severity/
        )
    ...
    context 'with invalid confidence' do
      it 'raises ReporterError for unknown confidence' do
        finding_bad_conf = FiberAudit::Finding.new(
          rule_id: 'test',
          category: 'test',
          severity: :high,
          confidence: :confirmed,
        ...
        finding_bad_conf.instance_variable_set(:@confidence, :unknown_level)
        bad_result = double('result', findings: [finding_bad_conf], suppressed: [], parse_errors: [], coverage: coverage, status: 'FAIL')
        expect { described_class.validate!(bad_result) }.to raise_error(
          FiberAudit::ReporterError,
          /invalid confidence/
        )
    ...
    context 'with invalid location' do
      it 'raises ReporterError for location without path' do
        bad_location = double('location', path: nil)
        ...
        bad_result = double('result', findings: [finding_bad_loc], suppressed: [], parse_errors: [], coverage: coverage, status: 'FAIL')
        expect { described_class.validate!(bad_result) }.to raise_error(
          FiberAudit::ReporterError,
          /invalid location/
        )
    ...
    context 'with non-JSON-safe details' do
      it 'raises ReporterError for evidence with non-JSON-safe details' do
        ...
        bad_result = double('result', findings: [finding_bad_ev], suppressed: [], parse_errors: [], coverage: coverage, status: 'FAIL')
        expect { described_class.validate!(bad_result) }.to raise_error(
          FiberAudit::ReporterError,
          /not JSON-safe/
        )
    ...
    context 'with invalid parse errors' do
      it 'raises ReporterError for parse error missing path' do
        ...
        bad_result = double('result', findings: [], suppressed: [], parse_errors: [bad_parse_error], coverage: coverage, status: 'FAIL')
        expect { described_class.validate!(bad_result) }.to raise_error(
          FiberAudit::ReporterError,
      ...
      it 'raises ReporterError for parse error with non-String path' do
        bad_parse_error = double('parse_error', path: 123, message: 'syntax error')
        bad_result = double('result', findings: [], suppressed: [], parse_errors: [bad_parse_error], coverage: coverage, status: 'FAIL')
        expect { described_class.validate!(bad_result) }.to raise_error(
          FiberAudit::ReporterError,
      ...
      it 'raises ReporterError for parse error with non-Integer line' do
        bad_parse_error = double('parse_error', path: 'test.rb', message: 'syntax error', line: '42')
        bad_result = double('result', findings: [], suppressed: [], parse_errors: [bad_parse_error], coverage: coverage, status: 'FAIL')
        expect { described_class.validate!(bad_result) }.to raise_error(
          FiberAudit::ReporterError,
          /line must be Integer or nil/
        )
      end

      it 'accepts parse error with nil line' do
        parse_error = double('parse_error', path: 'test.rb', message: 'syntax error', line: nil)
        good_result = double('result', findings: [], suppressed: [], parse_errors: [parse_error], coverage: coverage, status: 'FAIL')
        expect { described_class.validate!(good_result) }.not_to raise_error
      end
    end
    ...
    context 'with multiple findings' do
      it 'sorts findings by severity, rule_id, path, line, fingerprint' do
        ...
        hash = described_class.validate!(result_with_findings)
      ...
      it 'builds correct summary counts' do
        findings_multi = [
          FiberAudit::Finding.new(rule_id: 'r1', category: 'c', severity: :critical, confidence: :confirmed, message: 'm', evidence: evidence),
          FiberAudit::Finding.new(rule_id: 'r2', category: 'c', severity: :critical, confidence: :confirmed, message: 'm', evidence: evidence),
          FiberAudit::Finding.new(rule_id: 'r3', category: 'c', severity: :high, confidence: :confirmed, message: 'm', evidence: evidence),
          FiberAudit::Finding.new(rule_id: 'r4', category: 'c', severity: :medium, confidence: :confirmed, message: 'm', evidence: evidence),
          FiberAudit::Finding.new(rule_id: 'r5', category: 'c', severity: :low, confidence: :confirmed, message: 'm', evidence: evidence),
          FiberAudit::Finding.new(rule_id: 'r6', category: 'c', severity: :info, confidence: :confirmed, message: 'm', evidence: evidence)
        ]

        suppressed_multi = [
          FiberAudit::Finding.new(rule_id: 'r7', category: 'c', severity: :high, confidence: :confirmed, message: 'm', evidence: evidence)
        ]
        ...
        hash = described_class.validate!(result_multi)
        expect(hash[:summary]).to include(
          critical: 2,
          high: 1,
          medium: 1,
    ...
    context 'with Collection instead of Array' do
      it 'accepts Collection for findings' do
        ...
        hash = described_class.validate!(result_with_collection)
        expect(hash[:findings]).to be_an(Array)
        expect(hash[:findings].size).to eq(1)
      end
    end
    ...
    context 'with parse errors' do
      it 'preserves path, message, and nullable line' do
        ...
        hash = described_class.validate!(result_with_errors)
