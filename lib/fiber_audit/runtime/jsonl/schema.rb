...
module FiberAudit
  module Runtime
    module JSONL
      # Versioned record construction and validation remain one schema boundary.
      # rubocop:disable Metrics/ModuleLength
      module Schema
        SCHEMA_VERSION = Session::CURRENT_SCHEMA_VERSION
        SUPPORTED_SCHEMA_VERSIONS = Session::SCHEMA_VERSIONS
        RECORD_TYPES = %w[session_start event session_end].freeze
        ENVELOPE_KEYS = %w[schema_version record_type session_id sequence recorded_at monotonic_ns payload].freeze
        POLICY_KEYS = %w[redaction sampling_rate max_events_per_second max_events_per_session max_record_bytes
                         max_session_bytes fail_open].freeze
        START_KEYS = {
          '1.0' => %w[tool_version ruby_version policy].freeze,
          '1.1' => %w[tool_version ruby_version process_role policy].freeze
        }.freeze
        EVENT_KEYS = %w[kind source duration_ns operation location execution_context thread_id fiber_id measurements].freeze
        LOCATION_KEYS = %w[path line column].freeze
        END_KEYS = %w[status events_observed events_emitted dropped internal_errors].freeze
        DROPPED_KEYS = %w[sampling rate_limit session_event_limit session_byte_limit record_size_limit].freeze
        ...
        def start_record(session)
          ...
          payload = { 'tool_version' => session.tool_version, 'ruby_version' => session.ruby_version }
          payload['process_role'] = session.process_role.to_s if session.schema_version == '1.1'
          payload['policy'] = policy_payload(session.policy)
          build_record(
            schema_version: session.schema_version, record_type: 'session_start', session_id: session.id,
            sequence: 0, recorded_at: session.started_at,
            monotonic_ns: session.started_monotonic_ns, payload: payload
          )
        end

        def event_record(session_id:, sequence:, event:, schema_version: SCHEMA_VERSION)
          require_type!(event, Event, 'event')
          build_record(schema_version: schema_version, record_type: 'event', session_id: session_id, sequence: sequence,
                       recorded_at: event.occurred_at, monotonic_ns: event.monotonic_ns, payload: {
                         'kind' => event.kind.to_s, 'source' => event.source.to_s, 'duration_ns' => event.duration_ns,
                         'operation' => event.operation, 'location' => location_payload(event.location),
                         'execution_context' => event.execution_context.to_s, 'thread_id' => event.thread_id,
                         'fiber_id' => event.fiber_id, 'measurements' => event.measurements
                       })
        end

        def end_record(session_id:, sequence:, summary:, schema_version: SCHEMA_VERSION)
          require_type!(summary, SessionSummary, 'summary')
          build_record(
            schema_version: schema_version, record_type: 'session_end', session_id: session_id,
            sequence: sequence, recorded_at: summary.ended_at,
            monotonic_ns: summary.ended_monotonic_ns, payload: {
              'status' => summary.status.to_s, 'events_observed' => summary.events_observed,
              'events_emitted' => summary.events_emitted,
              'dropped' => { 'sampling' => summary.sampled_out, 'rate_limit' => summary.rate_limited,
        ...
        def validate!(record)
          ...
          schema_version = normalize_schema_version(record['schema_version'])
          ...
          when 'session_start' then validate_start_payload!(record['payload'], schema_version)
        ...
        def build_record(schema_version:, record_type:, session_id:, sequence:, recorded_at:, monotonic_ns:, payload:)
          record = { 'schema_version' => normalize_schema_version(schema_version), 'record_type' => record_type,
                     'session_id' => session_id.is_a?(String) ? session_id.dup : session_id, 'sequence' => sequence,
                     'recorded_at' => format_time(recorded_at), 'monotonic_ns' => monotonic_ns, 'payload' => payload }
          validate!(record)
          deep_freeze(record)
        end
        ...
        def policy_payload(policy)
          {
            'redaction' => policy.redaction.to_s, 'sampling_rate' => policy.sampling_rate,
            ...
            'max_session_bytes' => policy.max_session_bytes, 'fail_open' => policy.fail_open
          }
        end
        ...
        def validate_start_payload!(payload, schema_version)
          require_exact_keys!(payload, START_KEYS.fetch(schema_version), 'session_start payload')
          ...
          validate_process_role!(payload['process_role']) if schema_version == '1.1'
          require_exact_keys!(payload['policy'], POLICY_KEYS, 'policy')
          values = payload['policy']
          Policy.new(
            redaction: values['redaction'], sampling_rate: values['sampling_rate'],
            max_events_per_second: values['max_events_per_second'],
            max_events_per_session: values['max_events_per_session'],
            max_record_bytes: values['max_record_bytes'], max_session_bytes: values['max_session_bytes'],
        ...
        def normalize_schema_version(value)
          ...
          return normalized if SUPPORTED_SCHEMA_VERSIONS.include?(normalized)

          raise RuntimeContractError, "schema_version must be one of: #{SUPPORTED_SCHEMA_VERSIONS.join(', ')}"
        end
        private_class_method :normalize_schema_version
        def validate_process_role!(value)
          require_value!(value.is_a?(String) && Session::PROCESS_ROLES.any? do |role|
            value == role.to_s
          end, 'process_role is invalid')
        end
        private_class_method :validate_process_role!
        ...
        def validate_event_payload!(record)
          ...
          Event.new(
            ...
            monotonic_ns: record['monotonic_ns'], duration_ns: payload['duration_ns'],
            operation: payload['operation'], location: validate_location_payload!(payload['location']),
            execution_context: payload['execution_context'], thread_id: payload['thread_id'],
            fiber_id: payload['fiber_id'], measurements: payload['measurements']
          )
        end
        ...
        def validate_end_payload!(record)
          ...
          SessionSummary.new(
            ...
            ended_monotonic_ns: record['monotonic_ns'], status: payload['status'],
            events_observed: payload['events_observed'], events_emitted: payload['events_emitted'],
            sampled_out: dropped['sampling'], rate_limited: dropped['rate_limit'],
            session_event_limited: dropped['session_event_limit'],
            session_byte_limited: dropped['session_byte_limit'], oversize: dropped['record_size_limit'],
        ...
        def validate_uuid!(value)
          require_value!(value.is_a?(String) && value.match?(UUID),
        ...
        def parse_timestamp(value, field)
          require_value!(value.is_a?(String) && value.match?(TIMESTAMP),
        ...
        def format_time(value) = Validation.utc_time(value, 'recorded_at').iso8601(6)
        private_class_method :format_time
        def require_non_negative_integer!(value,
                                          field)
          require_value!(value.is_a?(Integer) && value >= 0,
        ...
        def deep_freeze(value)
          case value
          when Hash then value.each do |key, entry|
            deep_freeze(key)
            deep_freeze(entry)
          end
